...
export const wordHighlightPlugin = ViewPlugin.fromClass(class {
  ...
  update(update: ViewUpdate) {
    // Only rebuild if selection or document changed
    if (update.selectionSet || update.docChanged) {
      const word = this.getCurrentWord(update.view)

      // Rebuild only if the word changed or document changed
    ...
    for (const range of selection.ranges) {
      if (range.empty) {
        ...
        if (beforeMatch && afterMatch) {
          ...
          if (word.length > 0) {
            // Skip if cursor is on a property name (preceded by a dot)
  ...
  buildDecorations(view: EditorView, word: string) {
    ...
    // Build pattern once: match word not preceded by a dot
...
export const wordHighlightTheme = EditorView.baseTheme({
  '.cm-word-highlight': {
...
export function wordHighlightExtension() {
  return [
    wordHighlightPlugin,
    wordHighlightTheme,
  ]
